    ...
    <template v-if="tutorial">
        ...
        <div>
          <div class="modal-head-meta">
            <span class="category-chip" :class="`is-${normalizeText(tutorial.category).replace(/\s+/g, '-')}`">
              ...
              {{ displayCategory(tutorial.category) }}
            </span>
            <strong class="level-chip">{{ tutorial.level ? displayLevel(tutorial.level) : t('tutorials.popup.allLevels') }}</strong>
            <span
          ...
          <p v-if="tutorial.source === 'custom'" class="section-note modal-head-note">{{ t('tutorials.popup.savedLocally') }}</p>
        </div>
      </div>
      ...
      <div class="modal-layout">
        <div class="modal-media">
          ...
          <div v-else class="video-placeholder"><p>{{ t('tutorials.videoComingSoonLong') }}</p></div>
        </div>
        ...
        <div class="modal-copy">
          <div
            ...
            <section v-if="tutorial.cues?.length" class="coaching-card">
              <span class="coaching-kicker">{{ t('tutorials.popup.techniqueTips') }}</span>
              <ul class="coaching-list">
                <li v-for="cue in tutorialCues(tutorial)" :key="cue">{{ cue }}</li>
              </ul>
            </section>

            <section v-if="tutorial.steps?.length" class="coaching-card">
              <span class="coaching-kicker">{{ t('tutorials.popup.stepByStep') }}</span>
              <ol class="coaching-list coaching-list--ordered">
                <li v-for="step in tutorialSteps(tutorial)" :key="step">{{ step }}</li>
              </ol>
            </section>

            <section v-if="tutorial.mistakes?.length" class="coaching-card">
              <span class="coaching-kicker">{{ t('tutorials.popup.commonMistakes') }}</span>
              <ul class="coaching-list">
                <li v-for="mistake in tutorialMistakes(tutorial)" :key="mistake">{{ mistake }}</li>
              </ul>
            </section>
          </div>
          ...
          <section v-if="relatedTutorials.length" class="related-section">
            <div class="related-head">
              <span class="coaching-kicker">{{ t('tutorials.popup.relatedExercises') }}</span>
              <span class="related-note">{{ t('tutorials.popup.relatedNote') }}</span>
            </div>
            <div class="related-grid">
              <button
                ...
                <span class="related-top">
                  <span class="pill">{{ displayCategory(relatedTutorial.category) }}</span>
                  <span v-if="relatedTutorial.level" class="pill subtle">{{ displayLevel(relatedTutorial.level) }}</span>
                </span>
                ...
                <strong>{{ tutorialTitle(relatedTutorial) }}</strong>
                <span>{{ tutorialDescription(relatedTutorial) }}</span>
              </button>
            </div>
          </section>

          <p class="card-text modal-description">{{ tutorialDescription(tutorial) }}</p>

          <div class="modal-meta-grid">
            <div class="info-box info-box--category">
              <span class="category-label">{{ t('tutorials.category.label') }}</span>
              <div class="category-detail">
                ...
                <span class="category-note">{{ t('tutorials.popup.categoryNote') }}</span>
              </div>
            </div>
            <div class="info-box info-box--level">
              <div class="level-label-row">
                <span>{{ t('tutorials.popup.level') }}</span>
                <strong class="level-chip">{{ tutorial.level ? displayLevel(tutorial.level) : t('tutorials.popup.notSpecified') }}</strong>
              </div>
              <div class="level-visual" :class="`is-${(tutorial.level || 'none').toLowerCase()}`">
                <span v-if="tutorial.level" class="level-caption">{{ t('tutorials.popup.levelCaption') }}</span>
                <div v-if="tutorial.level" class="level-meter" :aria-label="tp('tutorials.popup.levelAria', { level: displayLevel(tutorial.level) })">
                  <span
                    v-for="bar in 3"
                    :key="bar"
                ...
                <div v-if="tutorial.level" class="level-scale" aria-hidden="true">
                  <span>{{ t('tutorials.popup.levelScaleEasy') }}</span>
                  <span>{{ t('tutorials.popup.levelScaleSolid') }}</span>
                  <span>{{ t('tutorials.popup.levelScaleElite') }}</span>
                </div>
              </div>
            </div>
            <div class="info-box info-box--muscles">
              <span class="muscle-label">{{ t('tutorials.popup.muscleGroups') }}</span>
              <div class="muscle-detail">
                <div class="muscle-list">
                  <strong
                    v-for="group in tutorialMuscleGroups(tutorial)"
                    :key="group"
                    class="muscle-pill">
                ...
                <span class="muscle-note">{{ t('tutorials.popup.muscleNote') }}</span>
              </div>
            </div>
            <div class="info-box modal-meta-card--full">
              <span class="equipment-label">{{ t('goals.composer.equipmentLabel') }}</span>
              <div class="equipment-detail">
                <div v-if="tutorial.equipment?.length" class="equipment-list">
                  <strong
                    v-for="item in tutorialEquipment(tutorial)"
                    :key="item"
                ...
                <strong v-else>{{ t('tutorials.popup.noEquipment') }}</strong>
                <span class="equipment-note">{{ t('tutorials.popup.equipmentNote') }}</span>
              </div>
            </div>
          </div>
        </div>
...
import { getTutorialLocalizedList, getTutorialLocalizedText } from '@/services/tutorialLocalization'
...
function translateDynamicText(value?: string | null) {
  return translateText(String(value ?? ''), locale.value)
}
...
function tutorialCues(tutorial: TutorialEntry) {
  return getTutorialLocalizedList(tutorial, 'cues', locale.value)
}

function tutorialSteps(tutorial: TutorialEntry) {
  return getTutorialLocalizedList(tutorial, 'steps', locale.value)
}

function tutorialMistakes(tutorial: TutorialEntry) {
  return getTutorialLocalizedList(tutorial, 'mistakes', locale.value)
}

function tutorialMuscleGroups(tutorial: TutorialEntry) {
  const localizedGroups = getTutorialLocalizedList(tutorial, 'muscleGroups', locale.value)
  if (localizedGroups.length) return localizedGroups
  return getTutorialMuscleGroups(tutorial).map((group) => translateDynamicText(group))
}

function tutorialEquipment(tutorial: TutorialEntry) {
  return getTutorialLocalizedList(tutorial, 'equipment', locale.value)
}
...
.modal-actions{position:sticky;bottom:0;z-index:3;justify-content:space-between;align-items:center;flex-wrap:nowrap;margin:1rem -1.1rem calc(-1rem - env(safe-area-inset-bottom, 0px));padding:.9rem 1.1rem calc(1rem + env(safe-area-inset-bottom, 0px));border-top:1px solid rgba(148,163,184,.18);background:color-mix(in srgb, var(--bg-card) 94%, #020617 6%);box-shadow:0 -18px 30px rgba(2,6,23,.08)}
.modal-actions::after{content:"";position:absolute;left:0;right:0;bottom:calc(-1rem - env(safe-area-inset-bottom, 0px));height:calc(1rem + env(safe-area-inset-bottom, 0px));background:inherit}
html.dark-mode .modal-actions{background:#020617;border-top-color:rgba(148,163,184,.5);box-shadow:0 -18px 30px rgba(0,0,0,.24)}
...
.tutorial-modal-actions-right{display:flex;flex-wrap:wrap;gap:.7rem;justify-content:flex-end}
@media (max-width: 900px){
  .related-grid,.modal-meta-grid{grid-template-columns:1fr}
  .modal-actions{justify-content:space-between}
  .tutorial-modal-actions-right{justify-content:flex-end;margin-left:auto}
}
</style>
