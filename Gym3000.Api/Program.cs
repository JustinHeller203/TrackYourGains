...
if (builder.Environment.IsProduction() && !connStr.Contains("SSL Mode", StringComparison.OrdinalIgnoreCase))
{
...
static string ToNpgsqlConnString(string url)
{
    ...
    if (!string.IsNullOrWhiteSpace(sslmode)) cs += $"SSL Mode={sslmode};";
    if (!cs.Contains("Trust Server Certificate", StringComparison.OrdinalIgnoreCase)) cs += "Trust Server Certificate=true;";
    return cs;
}
...
// ---- CORS (nur die von dir gesetzte AllowedOrigins-CSV) ----
    ...
    .AddJwtBearer(o =>
    {
        ...
        o.Events = new JwtBearerEvents
        {
            OnTokenValidated = async ctx =>
            {
                ...
                if (tvClaim != currentTv.ToString()) ctx.Fail("stale token");
            }
        };
    });
...
// Forwarded headers (hinter Proxy ok; bricht nichts)
...
// **KEINE** Prod-Security-Header, **KEIN** HSTS in diesem Test
